...
if($count>0){ 
   while ($row = mysqli_fetch_array($ret)) {
      ...
      $art_type = mysqli_real_escape_string($con, $row['typename']);
  
      // Check if the painting already exists
      $check_query = "SELECT * FROM paintings WHERE title = '$title' AND image_path = '$image'";
      $check_result = mysqli_query($con, $check_query);
  
      if (mysqli_num_rows($check_result) == 0) { // Only insert if not already in DB
          $insert_query = "INSERT INTO paintings (title, image_path, art_type, description) VALUES ('$title', '$image', '$art_type', 'Auto-stored artwork')";
